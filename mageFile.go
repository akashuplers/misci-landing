...
func StartProd() error {
	...
	os.Chdir("./backend")
	err = sh.RunV("pm2", "start", "npm", "--name", "prod1", "--", "run", "start")
	err = sh.RunV("pm2", "start", "npm", "--name", "prod2", "--", "run", "start1")
	specialFmtForFrontend := fmt.Sprintf("frontend")
	os.Chdir("../client")
	err = sh.RunV("pm2", "start", "npm", "--name", specialFmtForFrontend, "--", "run", "start")
...
func ServerStart(server string) error {
	...
	err = sh.RunV("pm2", "start", "npm", "--name", "api", "--", "run", "start")
	specialFmtForFrontend := fmt.Sprintf("frontend")
	os.Chdir("../client")
	err = sh.RunV("pm2", "start", "npm", "--name", specialFmtForFrontend, "--", "run", "start")
	fmt.Println("Reloading Caddy...")
